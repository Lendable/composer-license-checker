...
passthru(
    'composer require -W --no-interaction --no-progress --ansi '.($lowest ? '--prefer-lowest --prefer-stable ' : '').implode(' ', array_map(static fn (string $name): string => $name.':^'.$constraint, $names)),
    $resultCode,
);
